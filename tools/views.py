...
from .engine import (
    ...
    sexagesimal_convert, midpoint_slope_calc, circle_tangent_calc, solve_circle_angles, solve_inequality_steps,
    geometry_transform, statistics_summary, statistics_frequency, statistics_correlation
)
    ...
    def post(self, request):
        try:
            ...
            # New Modes for Grade 9
                ...
                result = geometry_transform(ttype, params, lang=lang)
            else:
        ...
        try:
            ...
            if sim_type in ['coin', 'dice', 'cards']:
                ...
                if mode == 'summary':
                    ...
                    result = statistics_summary(values, lang=lang)
                elif mode == 'frequency':
                    values = data.get('values') or []
                    bins = data.get('bins')
                    result = statistics_frequency(values, bins, lang=lang)
                elif mode == 'correlation':
                    x_vals = data.get('x') or []
                    y_vals = data.get('y') or []
                    result = statistics_correlation(x_vals, y_vals, lang=lang)
                else:
                    return JsonResponse({'error': 'Invalid statistics mode'}, status=400)
            else:
    ...
    def post(self, request):
        try:
            ...
            # New Modes for Grade 9
                ...
                if equation and any(op in equation for op in ['<', '>', '≤', '≥']):
                    result = solve_inequality_steps(equation, lang=lang)
                else:
